...
## Tracks
- **echo_grpo**: Qwen3-VL-2B, agentic, `echo` tool (select_view/select_frames/zoom). Config: `echo_verl/configs/echo_grpo.yaml`.
- **echo_ep**: frozen EchoPrime encoder (mvit_v2_s, 34.6M params, 512-dim out) → trainable projector → Qwen3-8B-text, no tools, no-SFT-cold-start ablation. Config: `echo_verl/configs/echoprime_grpo.yaml`.
...
- Disease mapping: `echo_verl/eval/diseases.py`. Their Table 1 numbers: `echo_verl/eval/echosonar_r.py`.
- One epoch = one QA pair per study, all studies (`--one-per-study --shuffle-seed N`).

## GRPO config
LoRA r=64/alpha=128 on attn+MLP only (not projector). lr=1e-6, KL loss on (coef 0.001). Reward: `echo_verl/reward.py` → `echo_rl/reward/score.py` (pooled, not EchoSonar-R's exact reward).
...
## Reward function (`echo_rl/reward/score.py`)
...
## grpo-echoprime-real-0913-2052 (echo_ep, no-SFT ablation) — active, paused at step 180
