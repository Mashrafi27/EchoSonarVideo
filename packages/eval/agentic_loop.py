...
# the harness, not the model. Training (verl_bridge/generate_trainset.py::_SYSTEM +
...
# mid-think episodes.
from verl_bridge.generate_trainset import _SYSTEM as SYSTEM_PROMPT   # noqa: E402

# The echo tool's OpenAI-function schema, loaded from the same YAML verl's rollout
# uses (verl_bridge/configs/echo_tool_config.yaml) so eval and training never drift.
def _load_echo_tool_schema():
    import pathlib
    import yaml
    p = pathlib.Path(__file__).resolve().parents[1] / "verl_bridge/configs/echo_tool_config.yaml"
...
def extract_think(text: str) -> str:
    """The model's <think>...</think> content, concatenated across turns --
    the actual reasoning trace, i.e. the opposite of strip_think(). Used by
    reasoning_quality.py (Table 2), which judges the reasoning, not the
    answer. Empty string if the model never produced a <think> block (a base
    model under plain-prompt mode has no such convention)."""
    if not isinstance(text, str):
        return ""
    blocks = re.findall(r"<think>(.*?)</think>", text, re.S)
    return " ".join(" ".join(b.split()) for b in blocks)
