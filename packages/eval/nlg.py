...
Implemented here: BLEU-1..4 (with the standard brevity penalty), ROUGE-L, and
METEOR (via nltk, the real metric -- not a home-rolled stand-in). BERTScore and
GREEN need models and live elsewhere:
  - BERTScore: `eval.bertscore` -- a contextual embedding model
    (PubMedBERT, as EchoSonar-R's Table 3 specifies).
  - GREEN [Ostmeier et al., arXiv 2405.03595]: LLM-judged clinical faithfulness
    from counts of significant errors, omissions and matched findings.
    EchoSonar-R uses an echocardiography-ADAPTED GREEN prompt judged by
    Mistral-7B that was never published, so we cannot reproduce THEIR number.
    `eval.green_approx` instead runs the original, PUBLIC GREEN
    prompt/formula (Stanford-AIMI/GREEN, arXiv 2405.03595) through our own
    judge model. This is a real implementation of the original metric, but it
    is NOT EchoSonar-R's adapted version -- never place it unlabeled next to
    their 0.800; every caller must tag it "GREEN (ours, public prompt)".

METEOR needs nltk's wordnet/omw-1.4/punkt data downloaded once:
...
def meteor(predictions: list, references: list) -> float:
    """Corpus-average METEOR via nltk's real implementation (synonym/stemming
    matching through WordNet) -- not a lexical-overlap stand-in.

    Raises with a clear message if the wordnet/punkt data isn't downloaded,
    rather than silently falling back to something that isn't METEOR.
    """
    ...
    scores = []
    for pred, ref in zip(predictions, references):
        p_tok, r_tok = word_tokenize(pred or ""), word_tokenize(ref or "")
        scores.append(meteor_score([r_tok], p_tok) if p_tok and r_tok else 0.0)
    return sum(scores) / len(scores)


def nlg_report(predictions: list, references: list) -> dict:
    """Everything we can compute without a judge model. BLEU/ROUGE-L/METEOR are
    all real implementations (nltk for METEOR). Deliberately does NOT include
    keys for BERTScore/GREEN -- those need a model/judge and live in
    `eval.bertscore` / `eval.green_approx`; an absent
    metric should be visibly absent rather than silently zero."""
    out = bleu_n(predictions, references)
    out["ROUGE-L"] = rouge_l_corpus(predictions, references)
    out["METEOR"] = meteor(predictions, references)
