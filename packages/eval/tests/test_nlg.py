...
from eval.nlg import bleu, bleu_n, meteor, nlg_report, rouge_l, rouge_l_corpus, tokenize
...
def test_meteor_is_present_bertscore_and_green_are_not():
    """METEOR is a real implementation (nltk) and belongs in nlg_report()'s
    output. BERTScore/GREEN need an actual model/judge and live in separate
    modules (eval.bertscore / eval.green_approx) -- a silent 0.0 for those
    here would read as a real measurement of a bad model."""
    out = nlg_report([GOLD], [GOLD])
    assert "METEOR" in out
    for absent in ("BERTScore", "GREEN"):
        assert absent not in out


def test_meteor_scores_near_one_for_identical_text():
    # nltk's real METEOR includes a fragmentation penalty even on an exact
    # match (chunk count > 1 for a multi-word sentence), so identical text
    # lands just under 1.0, not at it -- that's the genuine metric, not a bug.
    assert meteor([GOLD], [GOLD]) == pytest.approx(1.0, abs=0.01)
