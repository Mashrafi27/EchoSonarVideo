...
are `0.png, 1.png, ...` in native frame order -- data_core/data/studies.py:index_study).

Ported verbatim from /home/mashrafimonon/Mashrafi/EchoPrime (echo_prime/model.py's
process_mp4s + utils/utils.py's crop_and_scale) -- see echoprime_track/__init__.py for why this is
...
def study_to_tensor(study_dir: str) -> tuple:
    ...
    `di-<dicom_uuid>_<View>` directory name (data_core/data/studies.py's own convention).
    """
    clip_dirs = sorted(
    ...
    for clip_dir in clip_dirs:
        ...
        try:
            ...
            print(f"[echoprime_track] skipping {clip_dir}: {type(e).__name__}: {e}", flush=True)
    if not tensors:
