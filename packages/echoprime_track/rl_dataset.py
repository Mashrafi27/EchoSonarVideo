...
batch) -- no patch to verl's actor needed, only to the rollout (`echoprime_track/hf_rollout_video.py`),
...
class EchoPrimeGRPODataset(Dataset):
    ...
    def __init__(self, data_files, tokenizer, config, processor=None, max_samples: int = -1):
        ...
        assert self.view_token_id != tokenizer.unk_token_id, (
            ...
            "(see echoprime_track/train_sft.py's build_model_and_tokenizer / the checkpoint-prep script)")
