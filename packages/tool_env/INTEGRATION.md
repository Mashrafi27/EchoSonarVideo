# Phase-3 Integration Contract — wiring `tool_env` into VeRL

`tool_env` is runtime-free. Nothing below runs offline — it needs the torch/vLLM runtime.
...
- **Geometry (already applied to `tool_env`).** Qwen3-VL merged visual patch =
...
delegating to `data_core.reward.score.total_reward`.
...
offline against that pinned tree (`verl_bridge/tests/test_tool_config.py`), not assumed.
...
- **OPEN — initial-observation shape mismatch.** `data_core/sft/serialize.py` builds the SFT
  *user* turn as N per-view single-frame `{"type": "video"}` entries (the overview thumbnail
  strip), while the RL prompt built by `verl_bridge/generate_trainset.build_row` is a single
...
`external/DeepEyes/verl/workers/agent/envs/echo/tool_env.py`:

    from verl.workers.agent.tool_envs import ToolBase
    from tool_env import EchoEnv, EnvConfig, to_deepeyes_obs
