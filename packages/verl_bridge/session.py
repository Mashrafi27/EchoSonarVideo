...
Mirrors tool_env.EchoEnv._dispatch's op→tool mapping, but standalone and
...
"""
from tool_env.frames import PILFrameLoader
from tool_env.manifest import build_manifest
from tool_env.observation import Observation
from tool_env.tools import select_view, select_frames, zoom
...
class EchoSession:
    ...
    def run(self, op: str, params: dict) -> Observation:
        ...
        if op == "zoom":
            # bbox may be absent/None; tool_env.normalize_bbox guards non-len-4/None -> failure Observation.
