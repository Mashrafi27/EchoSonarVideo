...
- `rollout.multi_turn.tool_config_path=verl_bridge/configs/echo_tool_config.yaml` — Path to tool registry
...
- `custom_reward_function.path=verl_bridge/reward.py` — Reward module path
- `custom_reward_function.name=compute_score` — Entrypoint function name

## Dataset Configuration

Training data is generated from `verl_bridge.generate_trainset`:
...
- Row contract (per INTEGRATION.md §0.2, as emitted by `build_row`):
  ...
  - `reward_model` — `{"ground_truth": <JSON-encoded reward_key string>, "style": "rule"}`.
    ...
    targets are `list`). `verl_bridge/reward.py::compute_score` parses it back via
    `_parse_reward_key`.
  - `ability` — `"echo_vqa"`
...
3. Point dataset to parquet output of `verl_bridge.generate_trainset`
...
pydantic's `extra="ignore"`) also runs offline: `pytest verl_bridge/tests/test_tool_config.py`.
