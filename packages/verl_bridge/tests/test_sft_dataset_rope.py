...
offline `.venv` suite stays fast:
    ...
    .venv-train/bin/python -m pytest packages/verl_bridge/tests/test_sft_dataset_rope.py
...
def parquet():
    if not _PARQUET.exists():
        ...
        subprocess.run([sys.executable, str(_REPO / "packages" / "verl_bridge" / "generate_sft_parquet.py"),
...
def test_upstream_dataset_still_breaks_on_qwen3_vl(parquet, deps):
    """If this ever PASSES, verl fixed it upstream and verl_bridge/sft_dataset.py can go.

    Imported in a subprocess: verl_bridge.sft_dataset installs its shim into verl's
...
def test_echo_dataset_builds_qwen3_vl_position_ids(parquet, deps):
    from verl_bridge.sft_dataset import EchoMultiTurnSFTDataset
