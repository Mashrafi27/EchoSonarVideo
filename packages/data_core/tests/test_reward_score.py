from data_core.reward.score import (
...
def test_score_yesno_none_target_scores_zero():
    from data_core.reward.score import score_yesno, score_outcome
...
def test_scorers_null_safe_on_none_pred():
    from data_core.reward.score import score_yesno, score_set
    assert score_yesno(None, "yes") == 0.0
    assert score_set(None, ["x"]) == 0.0


def test_score_outcome_text_blends_judge_and_entity_f1():
    from data_core.reward.score import score_outcome, score_entity_f1, JudgeClient
...
def test_full_report_is_scored_per_section_not_as_a_blob():
    from data_core.reward.score import score_outcome
