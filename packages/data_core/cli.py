...
from data_core.config import Config
from data_core.data.studies import study_dir, index_study
from data_core.data.builders import iter_jsonl, sft_record, rl_record
from data_core.data.gold import load_all
from data_core.data.split import assign_split, load_canonical_split
from data_core.data.balance import resample_indices
from data_core.data.answers import last_answer, parse_yes_no
...
def build_eval(cfg, limit, split="test"):
    ...
    writes eval.jsonl. Same rl_record shape, so the scorers in data_core.reward.score
...
def run(argv=None):
    p = argparse.ArgumentParser(prog="data_core")
