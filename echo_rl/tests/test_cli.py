...
def _setup(tmp_path, study_uuids, gold_rows, missing_uuid=None):
...
def test_build_sft_and_rl(tmp_path, monkeypatch):
    env, out = _setup(tmp_path, ["st-1"], [("st-1", "TRAIN")])
...
def test_rl_pool_is_train_only(tmp_path, monkeypatch):
    # Force explicit train/val/test designations so the split assignment is deterministic.
