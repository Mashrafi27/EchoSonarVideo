...
Usage:
    python -m echo_ep.build_video_cache --studies-dir "$ECHO_PREPROCESSED_DIR" \\
...
from echo_ep.encoder import embed_videos, load_frozen_encoder
from echo_ep.preprocess import study_to_tensor
...
def main(argv=None) -> int:
    ...
    print(f"[echo_ep] {len(study_ids)} studies queued -> {args.out_dir}", flush=True)
    ...
    for i, study_uuid in enumerate(study_ids):
        ...
        try:
            ...
            failed += 1
            print(f"[echo_ep] FAILED {study_uuid}: {type(e).__name__}: {e}", flush=True)
        if (i + 1) % 50 == 0:
            elapsed = time.monotonic() - t0
            print(f"[echo_ep] {i + 1}/{len(study_ids)} "
                  f"(done={done} skipped={skipped} failed={failed}) "
                  f"{elapsed:.0f}s elapsed", flush=True)

    print(f"[echo_ep] finished: done={done} skipped={skipped} failed={failed}", flush=True)
    return 0
