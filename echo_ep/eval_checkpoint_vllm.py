...
`echo_ep/eval_checkpoint.py` (the original eval script) generates one example at a
...
Usage:
    python -m echo_ep.eval_checkpoint_vllm \
...
from echo_rl.data.answers import parse_yes_no
from echo_rl.reward.score import extract_answer
...
def merge_checkpoint(base_checkpoint: str, checkpoint_dir: str, merged_out_dir: str) -> None:
    """Same load path as echo_ep.eval_checkpoint (full FSDP state dict, not the
    ...
    shutil.copy(
        os.path.join(os.path.dirname(__file__), "modeling.py"),
...
def run_eval(args):
    ...
    from echo_ep.register_vllm_model import register_echoprime_vllm_model
