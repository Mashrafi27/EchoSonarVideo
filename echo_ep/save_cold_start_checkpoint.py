...
from echo_ep.train_sft import build_model_and_tokenizer
...
def main(argv=None) -> int:
    ...
    # just the pointer. modeling.py has no `echo_ep.*` relative imports, so copying it standalone
    # works.
    shutil.copy(os.path.join(os.path.dirname(__file__), "modeling.py"),
                os.path.join(args.out_dir, "modeling_echoprime_qwen3.py"))

    print(f"[echo_ep] cold-start checkpoint (no SFT) -> {args.out_dir}", flush=True)
    return 0
